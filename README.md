...
Claude Code may request very large outputs, such as `max_tokens=32000`. For hosted vLLM, the proxy caps output tokens to `HOSTED_VLLM_MAX_TOKENS` or `8192` by default so requests fit deployments with smaller `max_model_len` values.
...
3. Add environment variables in the Square Cloud dashboard:
   ...
   - optional: `HOSTED_VLLM_MAX_TOKENS=8192`
